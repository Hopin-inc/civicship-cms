...
export default ({ env }) => {
  ...
  const connection = {
    ...
    pool: {
      min: env.int('DB_CONNECTION_POOL_MIN', 0),
      max: env.int('DB_CONNECTION_POOL_MAX', 4),
      acquireTimeoutMillis: env.int('DB_CONNECTION_POOL_TIMEOUT', 60000),
      idleTimeoutMillis: env.int('DB_CONNECTION_POOL_IDLE_TIMEOUT', 30000),
      reapIntervalMillis: env.int('DB_CONNECTION_POOL_REAP_TIMEOUT', 1000),
      createRetryIntervalMillis: env.int('DB_CONNECTION_POOL_CREATE_ENTRY_INTERVAL', 200),
    },
